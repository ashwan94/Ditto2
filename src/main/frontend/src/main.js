...
export default function Main() {
    ...
    // 화상회의 플레이어에서 종료를 눌렀을 경우 DB 에 상태가 저장됨
    const goUpdate = async () => {
        const data = {
            onair: queryParams.onair,
            podcastBoardNo: queryParams.podcastBoardNo,
            status: 'N',
        }
        const res = await axios.post("/podcastBoard/update", data)
        if(res.status === 200){
            console.log("화상회의가 종료되었습니다.")
        }
    }

    // 화상회의 종료 후 메인 페이지 이동 시 query string 가져오기
    useEffect( () => {
        if(queryParams.onair){
           goUpdate();
        }
    }, [queryParams]);
